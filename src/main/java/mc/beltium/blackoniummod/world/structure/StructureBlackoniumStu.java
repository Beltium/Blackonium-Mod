...
package mc.beltium.blackoniummod.world.structure;
...
public class StructureBlackoniumStu extends ElementsBlackoniumMod.ModElement {
	public StructureBlackoniumStu(ElementsBlackoniumMod instance) {
		super(instance, 34);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldBlackoniumdiemsion.DIMID) {
			...
			isNetherType = WorldBlackoniumdiemsion.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 1000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("blackoniummod", "blackoniumstru"));
				if (template == null)
					return;
